...
function CardImage({event}) {
  ...
  function MyCardContent({event,setEvents}) {
    const unregister = async () => {
        const response = await axios.post('/api/registration/delete',{
    ...
    return (
      <div className="styleCardContent">
        ...
        <div>
            <button onClick={unregister} className='unregister'> Unregister </button>
        </div>
        <div>
...
export default function MyEventCard({event,setEvents}) {
